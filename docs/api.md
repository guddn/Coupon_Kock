...
응답은 매장, 정렬된 후보, 추천 옵션과 확인이 필요한 조건을 반환합니다. 등록 쿠폰과 공식 카드 문서 임베딩 검색 결과를 각각 결정적으로 계산해 비교하며, 공식 `source_id`가 있는 규칙만 카드 후보에 포함합니다. 이 엔드포인트는 Flutter용 결정적 계산 API이며 ADK 모델을 실행하지 않습니다.

### `GET /api/benefits/status`

RAG 저장소 종류, 임베딩 모델, 적재된 공식 문서 수를 반환합니다. MVP의 기대 문서 수는 3개입니다.

### `GET /api/benefits/search`

`canonical_brand`, `merchant_category`, `card_product`를 임베딩 질의로 만들고 코사인 유사도로 공식 카드 문서를 검색합니다. 응답에는 적용 규칙, 공식 출처 URL, 검색 점수와 임베딩 모델이 포함됩니다.
